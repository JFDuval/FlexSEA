...
volatile uint8 I2C_1_mstrStatus;     /* Master Status byte  */
volatile uint8 I2C_1_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8 * I2C_1_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8   I2C_1_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8   I2C_1_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8 * I2C_1_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8   I2C_1_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8   I2C_1_mstrWrBufIndex;   /* Master Write buffer Index      */


/*******************************************************************************
* Function Name: I2C_1_MasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_1_mstrStatus  - The global variable used to store a current
*                                 status of the I2C Master.
*  I2C_1_state       - The global variable used to store a current
*                                 state of the software FSM.
*  I2C_1_mstrControl - The global variable used to control the master
*                                 end of a transaction with or without Stop
*                                 generation.
*  I2C_1_mstrWrBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2C_1_mstrWrBufIndex - The global variable used to store current
*                                    index within the master write buffer.
*  I2C_1_mstrWrBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No
      ...
      
{
    ...
    if(NULL != wrData)
    {
        ...
        else if(I2C_1_SM_MSTR_HALT == I2C_1_state)
        {
            /* Master is ready and waiting for ReStart */
            errStatus = I2C_1_MSTR_NO_ERROR;

            I2C_1_ClearPendingInt();
            I2C_1_mstrStatus &= (uint8) ~I2C_1_MSTAT_XFER_HALT;
        }
        else
        {
            /* errStatus = I2C_1_MSTR_NOT_READY was send before */
        }

        if(I2C_1_MSTR_NO_ERROR == errStatus)
        {
...
/*******************************************************************************
* Function Name: I2C_1_MasterReadBuf
********************************************************************************
*
* Summary:
*  Automatically writes an entire buffer of data to a slave device. Once the
...
*                                 Stop generation.
*  I2C_1_mstrRdBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2C_1_mstrRdBufIndex - The global variable  used to store a
*                                    current index within the master
*                                    write buffer.
*  I2C_1_mstrRdBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(NULL != rdData)
    {
        ...
        {
            /* errStatus = I2C_1_MSTR_NOT_READY was set before */
        }
        ...
        if(I2C_1_MSTR_NO_ERROR == errStatus)
        {
            ...
            I2C_1_mstrStatus &= (uint8) ~I2C_1_MSTAT_RD_CMPLT;
...
/*******************************************************************************
* Function Name: I2C_1_MasterStatus
********************************************************************************
*
* Summary:
...
uint8 I2C_1_MasterStatus(void) 
{
    ...
    I2C_1_DisableInt(); /* Lock from interrupt */

    /* Read master status */
    status = I2C_1_mstrStatus;

    if (I2C_1_CHECK_SM_MASTER)
    {
        /* Set transfer in progress flag in status */
        status |= I2C_1_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: I2C_1_MasterClearStatus
********************************************************************************
*
* Summary:
...
uint8 I2C_1_MasterClearStatus(void) 
{
    ...
    status = I2C_1_mstrStatus;
    I2C_1_mstrStatus = I2C_1_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: I2C_1_MasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_1_mstrRdBufIndex - The global variable stores current index
...
/*******************************************************************************
* Function Name: I2C_1_MasterGetWriteBufSize
********************************************************************************
*
* Summary:
*  Returns the amount of bytes that has been transferred with an
...
*
* Global variables:
*  I2C_1_mstrWrBufIndex -  The global variable used to stores current
...
/*******************************************************************************
* Function Name: I2C_1_MasterClearReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_1_mstrRdBufIndex - The global variable used to stores current
...
/*******************************************************************************
* Function Name: I2C_1_MasterClearWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_1_mstrRdBufIndex - The global variable used to stote current
*                                    index within master read buffer.
*  I2C_1_mstrStatus - The global variable used to store a current
*                                status of the I2C Master.
*
