...
void CLK_100kHz_Start(void) 
{
    ...
    CLK_100kHz_CLKEN |= CLK_100kHz_CLKEN_MASK;
	CLK_100kHz_CLKSTBY |= CLK_100kHz_CLKSTBY_MASK;
}
...
void CLK_100kHz_Stop(void) 
{
    ...
    CLK_100kHz_CLKEN &= (uint8)(~CLK_100kHz_CLKEN_MASK);
	CLK_100kHz_CLKSTBY &= (uint8)(~CLK_100kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_100kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_100kHz_CLKEN & CLK_100kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_100kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_100kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_100kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_100kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_100kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_100kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_100kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_100kHz_GetSourceRegister();
    uint16 oldDivider = CLK_100kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_100kHz_CLKEN & CLK_100kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_100kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(CLK_100kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_100kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100kHz_MOD_SRC |= modeBitMask & (uint8)CLK_100kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_100kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_100kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_100kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_100kHz_MOD_SRC & (uint8)(CLK_100kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_100kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_100kHz_GetDividerRegister();
    uint8 oldSrc = CLK_100kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_100kHz_MOD_SRC |= CYCLK_SSS;
        CLK_100kHz_MOD_SRC =
            (CLK_100kHz_MOD_SRC & (uint8)(~CLK_100kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_100kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_100kHz_MOD_SRC & CLK_100kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_100kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100kHz_PHASE = clkPhase & CLK_100kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_100kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_100kHz_PHASE & CLK_100kHz_PHASE_MASK;
}
