...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_Start
********************************************************************************
*
* Summary:
...
void CyHALF_BUS_40MHZ_Start(void) 
{
    ...
    CyHALF_BUS_40MHZ_CLKEN |= CyHALF_BUS_40MHZ_CLKEN_MASK;
	CyHALF_BUS_40MHZ_CLKSTBY |= CyHALF_BUS_40MHZ_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_Stop
********************************************************************************
*
* Summary:
...
void CyHALF_BUS_40MHZ_Stop(void) 
{
    ...
    CyHALF_BUS_40MHZ_CLKEN &= (uint8)(~CyHALF_BUS_40MHZ_CLKEN_MASK);
	CyHALF_BUS_40MHZ_CLKSTBY &= (uint8)(~CyHALF_BUS_40MHZ_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyHALF_BUS_40MHZ_CLKEN & CyHALF_BUS_40MHZ_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyHALF_BUS_40MHZ_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyHALF_BUS_40MHZ_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyHALF_BUS_40MHZ__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyHALF_BUS_40MHZ_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyHALF_BUS_40MHZ_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyHALF_BUS_40MHZ_GetSourceRegister();
    uint16 oldDivider = CyHALF_BUS_40MHZ_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyHALF_BUS_40MHZ_CLKEN & CyHALF_BUS_40MHZ_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyHALF_BUS_40MHZ_DIV_PTR, clkDivider);
                CyHALF_BUS_40MHZ_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyHALF_BUS_40MHZ_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyHALF_BUS_40MHZ_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyHALF_BUS_40MHZ_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CyHALF_BUS_40MHZ_DIV_PTR, clkDivider);
				CyHALF_BUS_40MHZ_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyHALF_BUS_40MHZ_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyHALF_BUS_40MHZ_MOD_SRC |= modeBitMask & (uint8)CyHALF_BUS_40MHZ_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyHALF_BUS_40MHZ_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyHALF_BUS_40MHZ_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyHALF_BUS_40MHZ_MOD_SRC & (uint8)(CyHALF_BUS_40MHZ_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyHALF_BUS_40MHZ_GetDividerRegister();
    uint8 oldSrc = CyHALF_BUS_40MHZ_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyHALF_BUS_40MHZ_MOD_SRC |= CYCLK_SSS;
        CyHALF_BUS_40MHZ_MOD_SRC =
            (CyHALF_BUS_40MHZ_MOD_SRC & (uint8)(~CyHALF_BUS_40MHZ_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyHALF_BUS_40MHZ_MOD_SRC & CyHALF_BUS_40MHZ_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyHALF_BUS_40MHZ_PHASE = clkPhase & CyHALF_BUS_40MHZ_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyHALF_BUS_40MHZ_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyHALF_BUS_40MHZ_PHASE & CyHALF_BUS_40MHZ_PHASE_MASK;
}
