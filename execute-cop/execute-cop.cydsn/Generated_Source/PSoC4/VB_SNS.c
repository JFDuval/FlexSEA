...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        VB_SNS_PC =   (VB_SNS_PC & \
                                (uint32)(~(uint32)(VB_SNS_DRIVE_MODE_IND_MASK << (VB_SNS_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (VB_SNS_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(VB_SNS_DR & (uint8)(~VB_SNS_MASK));
    drVal = (drVal | ((uint8)(value << VB_SNS_SHIFT) & VB_SNS_MASK));
    VB_SNS_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(VB_SNS__0__SHIFT, mode);
}
...
{
    return (uint8)((VB_SNS_PS & VB_SNS_MASK) >> VB_SNS_SHIFT);
}
...
{
    return (uint8)((VB_SNS_DR & VB_SNS_MASK) >> VB_SNS_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(VB_SNS_INTSTAT & VB_SNS_MASK);
		VB_SNS_INTSTAT = maskedStatus;
        return maskedStatus >> VB_SNS_SHIFT;
    }
