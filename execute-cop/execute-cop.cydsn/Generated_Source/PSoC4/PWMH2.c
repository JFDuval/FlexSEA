...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        PWMH2_PC =   (PWMH2_PC & \
                                (uint32)(~(uint32)(PWMH2_DRIVE_MODE_IND_MASK << (PWMH2_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (PWMH2_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(PWMH2_DR & (uint8)(~PWMH2_MASK));
    drVal = (drVal | ((uint8)(value << PWMH2_SHIFT) & PWMH2_MASK));
    PWMH2_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(PWMH2__0__SHIFT, mode);
}
...
{
    return (uint8)((PWMH2_PS & PWMH2_MASK) >> PWMH2_SHIFT);
}
...
{
    return (uint8)((PWMH2_DR & PWMH2_MASK) >> PWMH2_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(PWMH2_INTSTAT & PWMH2_MASK);
		PWMH2_INTSTAT = maskedStatus;
        return maskedStatus >> PWMH2_SHIFT;
    }
