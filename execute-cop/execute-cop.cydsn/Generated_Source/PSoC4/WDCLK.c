...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        WDCLK_PC =   (WDCLK_PC & \
                                (uint32)(~(uint32)(WDCLK_DRIVE_MODE_IND_MASK << (WDCLK_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (WDCLK_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(WDCLK_DR & (uint8)(~WDCLK_MASK));
    drVal = (drVal | ((uint8)(value << WDCLK_SHIFT) & WDCLK_MASK));
    WDCLK_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(WDCLK__0__SHIFT, mode);
}
...
{
    return (uint8)((WDCLK_PS & WDCLK_MASK) >> WDCLK_SHIFT);
}
...
{
    return (uint8)((WDCLK_DR & WDCLK_MASK) >> WDCLK_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(WDCLK_INTSTAT & WDCLK_MASK);
		WDCLK_INTSTAT = maskedStatus;
        return maskedStatus >> WDCLK_SHIFT;
    }
